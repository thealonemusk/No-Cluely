document.addEventListener('DOMContentLoaded', () => {    
    ...
    const cursorKeyInput = document.getElementById('cursorKey');
    ...
    const loadSettingsIntoUI = (settings) => {
        ...
        if (claudeKeyInput) claudeKeyInput.value = settings.claudeKey || '';
        if (cursorKeyInput) cursorKeyInput.value = settings.cursorKey || '';
    ...
    const saveSettings = () => {
        ...
        if (claudeKeyInput) settings.claudeKey = claudeKeyInput.value;
        if (cursorKeyInput) settings.cursorKey = cursorKeyInput.value;
    ...
    const updateLlmFieldStates = () => {
        ...
        const claudeGroup = document.getElementById('claudeFields');
        const cursorGroup = document.getElementById('cursorFields');
        ...
        if (cursorGroup) cursorGroup.style.display = provider === 'cursor' ? '' : 'none';

        if (geminiKeyInput) geminiKeyInput.disabled = provider !== 'gemini';
        if (claudeKeyInput) claudeKeyInput.disabled = provider !== 'claude';
        if (cursorKeyInput) cursorKeyInput.disabled = provider !== 'cursor';
    };
