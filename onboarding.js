...
(function () {
  ...
  const LLM_COPY = {
    gemini: {
      ...
      placeholder: 'AIza…',
      hint: 'Don\'t have one? Get a free key at <a href="https://aistudio.google.com/apikey" target="_blank" rel="noreferrer">aistudio.google.com/apikey</a>. It\'s stored locally in <code>.env</code> — never sent anywhere except Google.',
      missing: 'Enter a Gemini API key',
      summary: 'Gemini API',
    },
    claude: {
      label: 'Anthropic API Key',
      placeholder: 'sk-ant-…',
      hint: 'Get a key at <a href="https://console.anthropic.com/" target="_blank" rel="noreferrer">console.anthropic.com</a>. Stored locally in <code>.env</code> — sent only to Anthropic.',
      missing: 'Enter a Claude API key',
      summary: 'Claude API',
    },
    cursor: {
      label: 'Cursor API Key',
      placeholder: 'key_…',
      hint: 'Get a key at <a href="https://cursor.com/dashboard" target="_blank" rel="noreferrer">cursor.com/dashboard</a>. Stored locally in <code>.env</code> — sent only to Cursor.',
      missing: 'Enter a Cursor API key',
      summary: 'Cursor API',
    },
  };
  ...
  function canAdvance() {
    ...
    switch (name) {
      ...
      case 'apikey':
        ...
        return !!state.geminiKey.trim() || isSelectedProviderConfigured();
      case 'speech':
  ...
  function isSelectedProviderConfigured() {
    if (state.llmProvider === 'claude') return state.claudeConfigured;
    if (state.llmProvider === 'cursor') return state.cursorConfigured;
    return state.geminiConfigured;
  }

  function applyLlmProviderCopy() {
    const copy = LLM_COPY[state.llmProvider] || LLM_COPY.gemini;
    if (apiKeyLabel) apiKeyLabel.textContent = copy.label;
    if (geminiInput) geminiInput.placeholder = copy.placeholder;
    if (apiKeyHint) apiKeyHint.innerHTML = copy.hint;
  }
  ...
  if (llmProviderSelect) {
    llmProviderSelect.addEventListener('change', () => {
      ...
      applyLlmProviderCopy();
      if (isSelectedProviderConfigured() && !state.geminiKey) {
  ...
  function populateSummary() {
    const rows = [];
    const copy = LLM_COPY[state.llmProvider] || LLM_COPY.gemini;
    const configured = !!(state.geminiKey || isSelectedProviderConfigured());
    rows.push({
      label: `<i class="fas fa-key"></i> ${copy.summary}`,
      value: configured ? 'Configured' : 'Missing',
      cls: configured ? 'ok' : 'skip',
    });
  ...
  $('#starBtn').addEventListener('click', () => {
    if (window.electronAPI && window.electronAPI.openExternal) {
      window.electronAPI.openExternal('https://github.com/thealonemusk/No-Cluely');
    } else {
      window.open('https://github.com/thealonemusk/No-Cluely', '_blank');
    }
  });
  ...
  nextBtn.addEventListener('click', async () => {
    ...
    if (!canAdvance()) {
      ...
      if (name === 'apikey') {
        const copy = LLM_COPY[state.llmProvider] || LLM_COPY.gemini;
        setKeyStatus('error', copy.missing);
      }
      return;
    }

    // Persist provider + key so the main overlay can initialize immediately.
    if (name === 'apikey' && window.electronAPI) {
      try {
        const payload = { llmProvider: state.llmProvider };
        if (state.geminiKey) {
          if (state.llmProvider === 'claude') payload.claudeKey = state.geminiKey;
          else if (state.llmProvider === 'cursor') payload.cursorKey = state.geminiKey;
          else payload.geminiKey = state.geminiKey;
        }
  ...
  if (window.electronAPI && window.electronAPI.getFirstRunStatus) {
    window.electronAPI.getFirstRunStatus().then((s) => {
      ...
      if (s.llmProvider) {
        ...
        if (llmProviderSelect) llmProviderSelect.value = s.llmProvider;
        applyLlmProviderCopy();
      }
      state.geminiConfigured = !!s.geminiConfigured;
      state.claudeConfigured = !!s.claudeConfigured;
      state.cursorConfigured = !!s.cursorConfigured;
