...
class ApplicationController {
  ...
  saveSettings(settings) {
    try {
      ...
      if (["gemini", "claude", "cursor"].includes(settings.llmProvider)) {
      ...
      if (settings.claudeKey !== undefined) {
        envUpdates.ANTHROPIC_API_KEY = settings.claudeKey;
      }
      ...
      const prevLlmProvider = process.env.LLM_PROVIDER || 'gemini';

      const persistedKeys = this.persistEnvUpdates(envUpdates);

      // Reinitialize the LLM client when the provider or any of its keys
      // change so Test connection / chat / screenshots use the new backend
      // without a restart.
      const llmProviderChanged = settings.llmProvider && settings.llmProvider !== prevLlmProvider;
      const llmKeyChanged = settings.geminiKey !== undefined
        || settings.claudeKey !== undefined
        || settings.cursorKey !== undefined;
      if (llmProviderChanged || llmKeyChanged) {
        try {
          llmService.initializeClient();
          logger.info("LLM service reinitialized after provider/key update", {
            provider: llmService.provider,
            llmProviderChanged,
            llmKeyChanged
          });
        } catch (e) {
          logger.warn("Failed to reinitialize LLM service after provider/key update", {
            error: e.message
          });
        }
      }
